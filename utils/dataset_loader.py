...
def prepare_dataset(df, model_type="cnn_lstm", use_cache=True):
    """
    ...
    Returns:
        tuple: (X, y) numpy arrays
            X shape depends on model_type:
                - cnn:      (n_samples, n_mfcc, time_frames, 1)
                - lstm:     (n_samples, time_frames, n_mfcc)
                - cnn_lstm: (n_samples, n_mfcc, time_frames, 1)
            y shape: (n_samples,) — integer class labels
    """
    import pickle
    
    cache_path = config.FEATURES_CACHE.replace(".pkl", f"_{model_type}.pkl")
    ...
    logger.info(f"Extracting features for {len(df)} audio files...")
    ...
    for idx, row in tqdm(df.iterrows(), total=len(df), desc="Extracting features"):
        try:
            ...
            audio, sr = preprocess_audio(
                ...
                apply_noise_reduction=False  # faster during batch processing
            )
    ...
    # Ensure uniform shape (pad/truncate time dimension)
...
def _get_target_time(feature_list, model_type):
    ...
    if model_type in ("cnn", "cnn_lstm"):
        times = [f.shape[1] for f in feature_list]  # (n_mfcc, time, 1)
    else:
        times = [f.shape[0] for f in feature_list]  # (time, n_mfcc)
...
def _fix_time_dim(features, target_time, model_type):
    ...
    if model_type in ("cnn", "cnn_lstm"):
        current_time = features.shape[1]  # (n_mfcc, time, 1)
        if current_time < target_time:
            pad = np.zeros((features.shape[0], target_time - current_time, 1), dtype=np.float32)
            features = np.concatenate([features, pad], axis=1)
        else:
            features = features[:, :target_time, :]
    else:  # lstm
        current_time = features.shape[0]  # (time, n_mfcc)
        if current_time < target_time:
            pad = np.zeros((target_time - current_time, features.shape[1]), dtype=np.float32)
